...
const CheckoutForm = () => {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(false); // Giả sử có logic kiểm tra đăng nhập
  ...
  useEffect(() => {
    const fetchCartAndBuyNow = async () => {
      ...
      if (isAuthenticated) {
        try {
          const response = await fetch('https://your-backend-api/api/cart', {
            headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
          });
          if (!response.ok) {
            console.error('Lỗi API cart:', await response.text());
            throw new Error('Lỗi khi lấy giỏ hàng');
          }
          const data = await response.json();
          console.log('Dữ liệu từ API cart:', data); // Debug dữ liệu từ API
        ...
        console.log('Dữ liệu từ localStorage:', localCart); // Debug dữ liệu localStorage
        currentCart = localCart ? JSON.parse(localCart) : [];
      }

      console.log('currentCart trước khi xử lý buyNowItem:', currentCart); // Debug trước khi xử lý
      if (buyNowItem) {
        const existingItem = currentCart.find(item => item.productId === buyNowItem.productId);
        if (existingItem) {
      ...
      console.log('currentCart sau khi xử lý buyNowItem:', currentCart); // Debug sau khi xử lý
      setCartItems([...currentCart]);
    };

    fetchCartAndBuyNow();
  }, [isAuthenticated, buyNowItem]);
  ...
  const handleSubmit = async (e) => {
    ...
    const orderData = {
      userId: isAuthenticated ? parseInt(localStorage.getItem('userId')) : null, // Lấy userId từ token nếu đã đăng nhập
      customerName,
      customerEmail,
      customerPhone,
      ...
      shippingProvince: provinces.find(p => p.code === parseInt(selectedProvince))?.name,
      shippingDistrict: districts.find(d => d.code === parseInt(selectedDistrict))?.name,
      shippingWard: wards.find(w => w.code === parseInt(selectedWard))?.name,
      paymentId: paymentMethods.find(method => method === selectedPaymentMethod) ? 1 : null,
...
// Kiểm tra orderItems
    if (orderData.orderItems.length === 0 || orderData.orderItems.some(item => item.productId === 0 || item.quantity === 0)) {
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        const remainingItems = cartItems.filter(item =>
          !orderData.orderItems.some(o => o.productId === item.productId && o.quantity === item.quantity)
        );
  ...
  // Tính tổng giá
  const totalPrice = cartItems.reduce((sum, item) => sum + (item.product?.price || item.price || 0) * item.quantity, 0);
          ...
          <ul className="space-y-2">
            {cartItems.map(item => (
              <li key={item.productId} className="flex justify-between items-center border-b py-2">
                ...
                <span>{(item.price * item.quantity || item.product?.price * item.quantity || 0).toLocaleString('vi-VN')} VND</span>
              </li>
            ))}
